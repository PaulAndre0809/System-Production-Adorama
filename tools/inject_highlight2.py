    ...
    try:
        ...
        for p in [wb_path, template_path]:
            ...
            try:
                ...
                retry(lambda: new_comp.CodeModule.AddFromString(VBA_CODE))
                ...
                if not found:
                    for i in range(1, tracker_comp.CodeModule.CountOfLines + 1):
                        ...
                        if "ApplyDashboardNativeFormulas wsD, dashboardTable" in line:
                            retry(lambda: tracker_comp.CodeModule.InsertLines(i + 1, "    Call ApplyDynamicScheduleFilterAndHighlight(dashboardTable)"))
