...
Private Sub BuildNotesInstructionSheet(ByRef wb As Workbook)
    ...
    On Error Resume Next
    Application.DisplayAlerts = False
    wb.Sheets("Notes - Instructions").Delete
    Application.DisplayAlerts = True
    On Error GoTo 0
    
    ' Create new sheet
    Set ws = wb.Sheets.Add(After:=wb.Sheets(wb.Sheets.Count))
    ws.Name = "Notes - Instructions"
    
    ' Apply basic sheet settings
    ActiveWindow.DisplayGridlines = False
    ws.Tab.Color = RGB(0, 32, 96) ' Dark Blue
    
    ' Title block
    With ws.Range("A1:E2")
        .Merge
        .Value = "System & Campaign Tracker Instructions"
        .Font.Name = "Arial"
        .Font.Size = 24
        .Font.Bold = True
        .Font.Color = RGB(255, 255, 255)
    ...
    ws.Range("D3").Value = "Dependencies/Notes"
    ws.Range("E3").Value = "Limitations"
    
    ' Add Instructions
    lastRow = 4
    
    ' Email Campaigns
    AddInstructionRow ws, lastRow, "Email Campaigns", "Add New Campaign", "Fill in details in the next available row. The 'Send Date' is crucial for calendar placement.", "Updates Calendar sheets automatically.", "Do not leave blank rows between campaigns."
    AddInstructionRow ws, lastRow, "Email Campaigns", "Change Campaign Status", "Update the 'Status' dropdown. Colors will auto-update based on selection.", "Status drives conditional formatting.", "Only use predefined statuses."
    AddInstructionRow ws, lastRow, "Email Campaigns", "Track Approvals", "Use 'Copy Approval' and 'Design Approval' columns to track sign-offs.", "None.", "None."
    
    ' SMS Campaigns
    AddInstructionRow ws, lastRow, "SMS Campaigns", "Add New Campaign", "Similar to Email, add details to the next available row on SMS sheet.", "Updates Calendar sheets automatically.", "Do not leave blank rows."
    AddInstructionRow ws, lastRow, "SMS Campaigns", "Update Status", "Change 'Status' column to reflect current state.", "Drives conditional formatting.", "None."
    
    ' Calendars
    AddInstructionRow ws, lastRow, "Calendars (Monthly)", "View Schedule", "Select the desired month tab (e.g., 'January'). The calendar populates automatically from Email/SMS sheets.", "Dependent on Send Dates in Email/SMS sheets.", "Do not manually edit calendar cells."
    AddInstructionRow ws, lastRow, "Calendars (Monthly)", "Navigate Months", "Use the 'Previous Month' and 'Next Month' buttons at the top of each calendar.", "Requires Macros to be enabled.", "Buttons only work if macros are enabled."
    
    ' Dashboard
    AddInstructionRow ws, lastRow, "Dashboard", "View Metrics", "Navigate to Dashboard sheet to see overall progress and status counts.", "Pulls data from Email and SMS sheets.", "Charts require manual refresh if pivot tables are used (though currently formula-based)."
    AddInstructionRow ws, lastRow, "Dashboard", "Refresh Data", "If any pivot tables exist, go to Data -> Refresh All.", "None.", "None."
    
    ' General System
    AddInstructionRow ws, lastRow, "General", "Formatting", "Click 'Apply All Configurations' button on Dashboard to re-apply standard styling.", "Overwrites manual formatting.", "May take a few seconds to run."
    AddInstructionRow ws, lastRow, "General", "Save/Backup", "Use standard Excel save features. Consider creating a backup copy periodically.", "None.", "None."
    
    ' Format as Table
    lastRow = ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
    Set tbl = ws.ListObjects.Add(xlSrcRange, ws.Range("A3:E" & lastRow), , xlYes)
    tbl.Name = "NotesTable"
    ...
    ' Protect Sheet
    ws.Protect Password:="adorama2024", UserInterfaceOnly:=True, AllowFiltering:=True
...
Private Sub AddInstructionRow(ByRef ws As Worksheet, ByRef rowNum As Long, ByVal comp As String, ByVal act As String, ByVal desc As String, ByVal dep As String, ByVal lim As String)
    ws.Cells(rowNum, 1).Value = comp
    ws.Cells(rowNum, 2).Value = act
    ws.Cells(rowNum, 3).Value = desc
    ws.Cells(rowNum, 4).Value = dep
    ws.Cells(rowNum, 5).Value = lim
    rowNum = rowNum + 1
End Sub
